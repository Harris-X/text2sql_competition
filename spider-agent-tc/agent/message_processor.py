        ...
        if matches:
            ...
            if func_match:
                ...
                if function_name == "execute_bash" and "work_dir" not in arguments:
                    # Align bash working directory with the exact schema folder used in prompts,
                    # which is <databases_path>/<db_id>/<db_id> (contains DDL.csv and table JSONs).
                    # Previously we pointed to one level up (<databases_path>/<db_id>),
                    # causing initial commands like `grep ... DDL.csv` to fail.
                    arguments["work_dir"] = os.path.join(self.args.databases_path, item['db_id'], item['db_id'])
