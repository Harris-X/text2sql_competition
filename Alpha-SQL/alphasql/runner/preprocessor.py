...
COST_RECORDER = CostRecorder(model="Qwen3-Coder-30B-A3B-Instruct")
...
class Preprocessor:
    ...
    def __init__(self, 
        ...
        # Check for user's custom format
        if self.data and "input" in self.data[0] and "output" in self.data[0]:
            self.tasks = []
            # Use a default db_id for MySQL
            default_db_id = "mysql_db" 
            self.all_db_ids = [default_db_id]
            
            for i, data_item in enumerate(self.data):
                question, db_schema, evidence = extract_all_sections(data_item["input"])
                
                self.tasks.append(Task(
                    question_id=data_item.get("question_id", i),
                    db_id=default_db_id,
                    question=question,
                    evidence=evidence,
                    sql=data_item["output"],
                    difficulty="medium"
                ))
        else:
