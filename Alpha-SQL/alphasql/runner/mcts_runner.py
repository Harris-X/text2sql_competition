...
# 禁用 Weave/WandB 以避免 protobuf 版本冲突，这些仅用于可视化日志，
# 对 MCTS 核心逻辑无影响。
# try:
#     import weave
...
class MCTSRunner:
    ...
    def run_all_tasks(self):
        ...
        # Optional task limit for debugging (env: ALPHASQL_TASK_LIMIT)
        task_limit_env = os.getenv("ALPHASQL_TASK_LIMIT")
        if task_limit_env and task_limit_env.isdigit():
            limit = int(task_limit_env)
            if limit > 0:
                tasks = tasks[:limit]
                print(f"Applying task limit: {limit}. Now {len(tasks)} tasks to solve")

        # If only 1 process requested, run sequentially to ease debugging / avoid multiprocessing overhead
        if self.config.n_processes == 1:
            for t in tqdm(tasks, total=len(tasks), desc="Solving tasks (sequential)"):
                self.run_one_task(t)
        else:
