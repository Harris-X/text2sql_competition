...
class LSHIndex:
    ...
    def get_unique_database_values(cls, database_schema: DatabaseSchema, ignore_primary_keys: bool = True, ignore_non_text_columns: bool = True) -> Dict[str, Dict[str, List[str]]]:
        ...
        # If using cloud MySQL (mysql_db), query distinct values via MySQL; else use local sqlite
        use_mysql = database_schema.db_id == "mysql_db"
        mysql_conn = None
        if use_mysql:
            try:
                mysql_conn = _get_mysql_conn()
            except Exception as e:
                # Fallback to sqlite if MySQL not reachable
                use_mysql = False
        ...
        for table_name, table_schema in database_schema.tables.items():
            for column_name, column_schema in table_schema.columns.items():
                if ignore_primary_keys and column_schema.primary_key:
                    continue
                # Support MySQL text types
                is_text = ("text" in column_schema.column_type.lower() or "char" in column_schema.column_type.lower())
                if ignore_non_text_columns and not is_text:
                    continue
                if use_mysql and mysql_conn is not None:
                    with mysql_conn.cursor() as cursor:
                        cursor.execute(f"SELECT DISTINCT `{column_name}` FROM `{table_name}` WHERE `{column_name}` IS NOT NULL AND `{column_name}` != '' LIMIT 1000;")
                        rows = cursor.fetchall()
                        vals = [str(list(r.values())[0]) for r in rows if list(r.values())[0] is not None]
                        if vals:
                            unique_values[table_name][column_name] = vals
                else:
