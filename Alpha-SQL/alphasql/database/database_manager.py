...
from alphasql.database.utils import load_database_schema_dict, build_table_ddl_statement, load_database_schema_dict_from_mysql
    ...
    def get_database_schema(cls, db_id: str, database_root_dir: str) -> DatabaseSchema:
        ...
        if db_id not in cls.CACHED_DATABASE_SCHEMA:
            with cls._lock:  # Add lock protection
                ...
                if db_id not in cls.CACHED_DATABASE_SCHEMA:
                    if db_id == "mysql_db":
                        database_schema_dict = load_database_schema_dict_from_mysql(db_id)
                    else:
