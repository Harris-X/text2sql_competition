...
def execute_sql_with_timeout(db_path: str, query: str, timeout: int = 60) -> SQLExecutionResult:
    ...
    if "mysql_db" in str(db_path):
        return execute_sql_with_pymysql(query, timeout)
...
def execute_sql_without_timeout(db_path: str, query: str) -> SQLExecutionResult:
    ...
    # Check if we should use MySQL
    if "mysql_db" in str(db_path):
        return execute_sql_with_pymysql(query)
...
def normalize_sql(sql: str) -> str:
    ...
    try:
        ...
        return parsed.sql(dialect="mysql", normalize=True, pretty=False, comments=False)
    except Exception as e:
...
def execute_sql_with_pymysql(query: str, timeout: int = 60) -> SQLExecutionResult:
    """
    Execute SQL using pymysql.
    """
    db_config = {
        ...
        'cursorclass': pymysql.cursors.Cursor # Use default cursor to get tuple results like sqlite3
    }
    
    try:
        conn = pymysql.connect(**db_config)
        # Set timeout if possible, or handle via thread/process if needed. 
        # pymysql doesn't have a simple query timeout param in connect, but we can set session variable.
        # cursor.execute(f"SET STATEMENT_TIMEOUT = {timeout * 1000}") # MySQL 5.7.8+
        
        with conn.cursor() as cursor:
            cursor.execute(query)
            result = cursor.fetchall()
            description = cursor.description
            result_cols = [col[0] for col in description] if description else []
            
            # Normalize types to match sqlite3 output (tuples of values)
            # pymysql returns tuples if using default Cursor.
            
            return SQLExecutionResult("mysql_db", query, SQLExecutionResultType.SUCCESS, result_cols, list(result), None)
            
    except Exception as e:
        return SQLExecutionResult("mysql_db", query, SQLExecutionResultType.ERROR, None, None, str(e))
    finally:
