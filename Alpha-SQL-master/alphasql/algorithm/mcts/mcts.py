...
class MCTSSolver:
    ...
    def backpropagate(self, node: MCTSNode):
        logger = get_logger()
        logger.info(f"Backpropagate start final_sql={repr(node.final_sql_query)[:200]}")
        current = node
        if current.N == 0:
        ...
        logger.info(f"Backpropagate done reward={reward}")
    ...
    def solve(self):
        logger = get_logger()
        logger.info(f"Solve start task_id={self.task.question_id} max_rollout={self.max_rollout_steps}")
        ...
        for rollout_i in range(self.max_rollout_steps):
            logger.info(f"Rollout task={self.task.question_id} step={rollout_i+1}/{self.max_rollout_steps}")
            leaf_node = self.select(root_node)
            logger.debug(f"Selected leaf type={leaf_node.node_type} depth={leaf_node.depth} children={len(leaf_node.children)} N={leaf_node.N}")
            if leaf_node.is_terminal():
                logger.info("Leaf already terminal; skipping expansion")
                self.backpropagate(leaf_node)
                continue
            self.expand(leaf_node)
            logger.debug(f"Expanded leaf now children={len(leaf_node.children)}")
            leaf_node = random.choice(leaf_node.children)
            logger.debug(f"Random child chosen type={leaf_node.node_type} depth={leaf_node.depth}")
            end_node = self.simulate(leaf_node)
            logger.info(f"Simulation reached terminal depth={end_node.depth}")
        ...
        logger.info(f"Task={self.task.question_id} done valid_reasoning_paths={len(all_valid_reasoning_paths)} saving={save_path}")
